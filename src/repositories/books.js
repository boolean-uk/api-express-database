...
const getAllBooksByTypeOrTopic = async (
  ...
  let filters = "";
  let authorQuery = "";
  let per_page_number = 20;
  let pageNumber = 1;
  // if statements for type/topic/author
  if (author) {
    authorQuery = "AND author = $1";
  }

  if (per_page) {
    per_page_number = `LIMIT $1`;
  }
  if (page) {
    offsetNumber = page * per_page;

    return (pageNumber = offsetNumber);
  }
  ...
  if (type && topic) {
    ...
    filters = filters.concat(` ${authorQuery}`);
    filters = filters.concat(` ${pageNumber}`);
    filters = filters.concat(` ${per_page_number}`);

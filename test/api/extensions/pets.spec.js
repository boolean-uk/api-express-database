...
describe('Pets Endpoint', () => {
    describe('POST /pets', () => {
        it('will return 400 if missing fields', async () => {
            const response = await supertest(app)
                .post('/pets')
                .send({ name: 'Joey', type: 'dog' });

            expect(response.status).toEqual(400);
            expect(response.body.error).toEqual(
                `missing fields: age, breed, microchip`
            );
        });
    });
    ...
    describe('GET /pets Pagination', () => {
        ...
        it('will return page 2 only of 30 pets', async () => {
            ...
            expect(
                response.body.pets[response.body.pets.length - 1].id
            ).toEqual(60);
        });
        ...
        it('will return 400 if pagination parameters are invalid', async () => {
            ...
            expect(response.status).toEqual(400);
            expect(response.body.error).toEqual(
                `parameter invalid perPage: 9 not valid. Accepted range is 10 - 50`
            );
        });

        it('will return 400 if pagination parameters are invalid', async () => {
            const response = await supertest(app).get(
                `/pets?page=1&perPage=51`
            );

            expect(response.status).toEqual(400);
            expect(response.body.error).toEqual(
                `parameter invalid perPage: 51 not valid. Accepted range is 10 - 50`
            );
        });
